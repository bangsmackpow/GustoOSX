...
import { BarChart3, Download, Printer, TrendingUp, DollarSign, Receipt, CreditCard, Banknote } from "lucide-react";
...
export function ReportsView() {
  ...
  const loadReport = async () => {
    setLoading(true);
    try {
      // Get closed tabs and their orders for today
      const tabsResult = await window.api.db.query(
        "SELECT * FROM tabs WHERE status = 'closed' AND closed_at >= unixepoch('now', 'start of day')",
        []
      );
    ...
    <div className="flex-1 p-8 overflow-y-auto bg-pos-bg">
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tighter">Z-Report</h1>
            <p className="text-pos-text-muted text-sm mt-1">End of shift summary for {new Date().toLocaleDateString()}</p>
          </div>
        ...
        {/* Item Breakdown */}
        <div className="bg-pos-surface border border-pos-border rounded-3xl overflow-hidden shadow-xl">
              ...
              <tbody className="divide-y divide-pos-border/50">
                {reportData?.items.map((item: any, idx: number) => (
                  <tr key={idx} className="hover:bg-pos-surface-hover/50 transition-colors">
                    <td className="px-6 py-4 text-sm font-medium">{item.drink_name}</td>
                    <td className="px-6 py-4 text-sm text-right tabular-nums">{item.total_qty}</td>
                    <td className="px-6 py-4 text-sm text-right font-bold text-pos-accent tabular-nums">${item.total_rev.toFixed(2)}</td>
                  </tr>
                ))}
                {reportData?.items.length === 0 && (
                  <tr>
                    <td colSpan={3} className="px-6 py-12 text-center text-pos-text-muted italic text-sm">
...
function StatCard({ title, value, icon: Icon, color }: { title: string, value: string, icon: any, color: string }) {
  return (
    <div className="bg-pos-surface border border-pos-border p-6 rounded-3xl shadow-sm hover:border-pos-accent/30 transition-all group">
